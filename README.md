...
1. **Install Firebase CLI (`firebase-tools`)**
   ...
   - **Metode Windows Package Manager (Recommended):**

     ```bash
     # Install Node.js dulu jika belum ada
     winget install OpenJS.NodeJS

     # Restart terminal atau update PATH
     # Lalu install Firebase CLI
     npm install -g firebase-tools
     ```

   - **Metode npm (Node.js) - Linux/macOS:**
     ```bash
     sudo apt update
     sudo apt install nodejs npm
     sudo npm install -g firebase-tools
     ```
   - **Metode script resmi (Linux/macOS):**
     ```bash
     curl -sL https://firebase.tools | bash
     # atau jika membutuhkan akses root
     sudo curl -sL https://firebase.tools | bash
     ```
   - **Windows: Jika `firebase` command tidak ditemukan setelah install npm:**

     ```powershell
     # Cara 1: Tambahkan ke PATH untuk session saat ini
     $env:PATH += ";C:\Program Files\nodejs\;$env:APPDATA\npm"

     # Cara 2: Set PATH permanen (run as Administrator)
     setx PATH "%PATH%;C:\Program Files\nodejs\;%APPDATA%\npm" /M

     # Cara 3: Restart VS Code atau buka terminal baru
     # Cara 4: Gunakan full path jika masih bermasalah
...
2. **Login ke Firebase**
   ...
   ```bash
   # Jika firebase command tidak ditemukan di Windows:
   $env:PATH += ";C:\Program Files\nodejs\;$env:APPDATA\npm"
   ...
   **Troubleshooting Windows:** Jika tetap error, gunakan:
